...
public class EdgarStartUpService : IHostedService
{
    ...
    private readonly List<int> _ciks;

    public EdgarStartUpService(IServiceProvider serviceProvider)
    {
        _serviceProvider = serviceProvider;
        _ciks = new List<int> { 18926,892553,1510524,1858912,1828248,1819493,60086,1853630,1761312,1851182,
                                ...
                                884624,1501103,1397183,1552797,1894630,823277,21175,1439124,52827,1730773,1867287,
                                1685428,1007587,92103,1641751,6845,1231457,947263,895421,1988979,1848898,844790,1541309,
    ...
    public async Task StartAsync(CancellationToken cancellationToken)
    {
        ...
        {
            var edgarService = scope.ServiceProvider.GetRequiredService<EdgarService>();
            await edgarService.FetchAndSaveDataAsync(_ciks);
        }
    }
